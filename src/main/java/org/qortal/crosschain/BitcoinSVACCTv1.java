 ...
 * <p>
 * <ul>
 * <li>Bob generates BitcoinSV & Qortal 'trade' keys, and secret-b
 * 		<ul>
 ...
 * 		<ul>
 * 			<li>Alice generates BitcoinSV & Qortal 'trade' keys</li>
 * 			<li>Alice funds BitcoinSV P2SH-A</li>
 * 			<li>Alice sends 'offer' MESSAGE to Bob from her Qortal trade address, containing:
 * 				<ul>
 * 					<li>hash-of-secret-A</li>
 * 					<li>her 'trade' BitcoinSV PKH</li>
 * 				</ul>
 * 			</li>
 * 		</ul>
 * </li>
 ...
 * 					<li>Alice's trade BitcoinSV PKH</li>
 * 					<li>hash-of-secret-A</li>
 * 				</ul>
 * 			</li>
 * 		</ul>
 ...
 * 		<ul>
 * 			<li>Alice creates/funds BitcoinSV P2SH-B</li>
 * 		</ul>
 * </li>
 * <li>Bob checks P2SH-B is funded
 * 		<ul>
 * 			<li>Bob redeems P2SH-B using his BitcoinSV trade key and secret-B</li>
 * 		</ul>
 * </li>
 ...
 * 		<ul>
 * 			<li>Bob redeems P2SH-A using his BitcoinSV trade key and secret-A</li>
 * 			<li>P2SH-A BSV funds end up at BitcoinSV address determined by redeem transaction output(s)</li>
 * 		</ul>
 * </li>
 * </ul>
 */
public class BitcoinSVACCTv1 implements ACCT {

	public static final String NAME = BitcoinSVACCTv1.class.getSimpleName();
	...
	public static class OfferMessageData {
		public byte[] partnerBitcoinSVPKH;
		public byte[] hashOfSecretA;
		public long lockTimeA;
	}
	public static final int OFFER_MESSAGE_LENGTH = 20 /*partnerBitcoinSVPKH*/ + 20 /*hashOfSecretA*/ + 8 /*lockTimeA*/;
	public static final int TRADE_MESSAGE_LENGTH = 32 /*partner's Qortal trade address (padded from 25 to 32)*/
			+ 24 /*partner's BitcoinSV PKH (padded from 20 to 24)*/
			+ 8 /*lockTimeB*/
	...
	private static BitcoinSVACCTv1 instance;

	private BitcoinSVACCTv1() {
	}

	public static synchronized BitcoinSVACCTv1 getInstance() {
		if (instance == null)
			instance = new BitcoinSVACCTv1();
	 ...
	 * @param bitcoinSVPublicKeyHash 20-byte HASH160 of creator's trade BitcoinSV public key
	 * @param hashOfSecretB 20-byte HASH160 of 32-byte secret-B
	 * @param qortAmount how much QORT to pay trade partner if they send correct 32-byte secrets to AT
	 * @param bitcoinSVAmount how much BSV the AT creator is expecting to trade
	 * @param tradeTimeout suggested timeout for entire trade
	 */
	public static byte[] buildQortalAT(String creatorTradeAddress, byte[] bitcoinSVPublicKeyHash, byte[] hashOfSecretB, long qortAmount, long bitcoinSVAmount, int tradeTimeout) {
		...
		final int addrBitcoinSVPublicKeyHash = addrCounter;
		addrCounter += 4;
		...
		final int addrBitcoinSVAmount = addrCounter++;
		...
		final int addrTradeMessagePartnerBitcoinSVPKHOffset = addrCounter++;
		final int addrPartnerBitcoinSVPKHPointer = addrCounter++;
		...
		final int addrPartnerBitcoinSVPKH = addrCounter;
		addrCounter += 4;
		...
		// BitcoinSV public key hash
		assert dataByteBuffer.position() == addrBitcoinSVPublicKeyHash * MachineState.VALUE_SIZE : "addrBitcoinSVPublicKeyHash incorrect";
		dataByteBuffer.put(Bytes.ensureCapacity(bitcoinSVPublicKeyHash, 32, 0));
		...
		// Expected BitcoinSV amount
		assert dataByteBuffer.position() == addrBitcoinSVAmount * MachineState.VALUE_SIZE : "addrBitcoinSVAmount incorrect";
		dataByteBuffer.putLong(bitcoinSVAmount);
		...
		// Offset into 'trade' MESSAGE data payload for extracting partner's BitcoinSV PKH
		assert dataByteBuffer.position() == addrTradeMessagePartnerBitcoinSVPKHOffset * MachineState.VALUE_SIZE : "addrTradeMessagePartnerBitcoinSVPKHOffset incorrect";
		dataByteBuffer.putLong(32L);

		// Index into data segment of partner's BitcoinSV PKH, used by GET_B_IND
		assert dataByteBuffer.position() == addrPartnerBitcoinSVPKHPointer * MachineState.VALUE_SIZE : "addrPartnerBitcoinSVPKHPointer incorrect";
		dataByteBuffer.putLong(addrPartnerBitcoinSVPKH);
		...
		for (int pass = 0; pass < 2; ++pass) {
			...
			try {
				...
				// Extract trade partner's BitcoinSV public key hash (PKH)
				codeByteBuffer.put(OpCode.EXT_FUN_DAT.compile(QortalFunctionCode.PUT_PARTIAL_MESSAGE_FROM_TX_IN_A_INTO_B.value, addrTradeMessagePartnerBitcoinSVPKHOffset));
				// Extract partner's BitcoinSV PKH (we only really use values from B1-B3)
				codeByteBuffer.put(OpCode.EXT_FUN_DAT.compile(FunctionCode.GET_B_IND, addrPartnerBitcoinSVPKHPointer));
				...
				throw new IllegalStateException("Unable to compile BSV-QORT ACCT?", e);
			}
		}
		...
		assert Arrays.equals(Crypto.digest(codeBytes), BitcoinSVACCTv1.CODE_BYTES_HASH)
			: String.format("BSVACCT.CODE_BYTES_HASH mismatch: expected %s, actual %s", HashCode.fromBytes(CODE_BYTES_HASH), HashCode.fromBytes(Crypto.digest(codeBytes)));
	...
	public CrossChainTradeData populateTradeData(Repository repository, byte[] creatorPublicKey, long creationTimestamp, ATStateData atStateData) throws DataException {
		...
		tradeData.foreignBlockchain = SupportedBlockchain.BITCOINSV.name();
		...
		// Creator's BitcoinSV/foreign public key hash
		...
		// Skip 'trade' message data offset for partner's bitcoinSV PKH
		dataByteBuffer.position(dataByteBuffer.position() + 8);

		// Skip pointer to partner's bitcoinSV PKH
		dataByteBuffer.position(dataByteBuffer.position() + 8);
		...
		// Potential partner's BitcoinSV PKH
		byte[] partnerBitcoinSVPKH = new byte[20];
		dataByteBuffer.get(partnerBitcoinSVPKH);
		dataByteBuffer.position(dataByteBuffer.position() + 32 - partnerBitcoinSVPKH.length); // skip to 32 bytes
		...
		if (acctMode != null && acctMode != AcctMode.OFFERING) {
			...
			tradeData.partnerForeignPKH = partnerBitcoinSVPKH;
	...
	public static byte[] buildOfferMessage(byte[] partnerBitcoinSVPKH, byte[] hashOfSecretA, int lockTimeA) {
		byte[] lockTimeABytes = BitTwiddling.toBEByteArray((long) lockTimeA);
		return Bytes.concat(partnerBitcoinSVPKH, hashOfSecretA, lockTimeABytes);
	}
	...
	public static OfferMessageData extractOfferMessageData(byte[] messageData) {
		...
		offerMessageData.partnerBitcoinSVPKH = Arrays.copyOfRange(messageData, 0, 20);
	...
	public static byte[] buildTradeMessage(String partnerQortalTradeAddress, byte[] partnerBitcoinSVPKH, byte[] hashOfSecretA, int lockTimeA, int lockTimeB) {
		...
		System.arraycopy(partnerBitcoinSVPKH, 0, data, 32, partnerBitcoinSVPKH.length);
