 ...
 * </ul>
 */
public class BitcoinCashACCTv1TradeBot implements AcctTradeBot {

	private static final Logger LOGGER = LogManager.getLogger(BitcoinCashACCTv1TradeBot.class);
	...
	private static BitcoinCashACCTv1TradeBot instance;
	...
	private BitcoinCashACCTv1TradeBot() {
	}

	public static synchronized BitcoinCashACCTv1TradeBot getInstance() {
		if (instance == null)
			instance = new BitcoinCashACCTv1TradeBot();
	...
	/**
	 * Creates a new trade-bot entry from the "Bob" viewpoint, i.e. OFFERing QORT in exchange for BCH.
	 * <p>
	 * Generates:
	 * <ul>
	 ...
	 * 	<li>'foreign' (as in BitcoinCash) public key, public key hash</li>
	 *	<li>HASH160 of secret-B</li>
	 * </ul>
	 * A Qortal AT is then constructed including the following as constants in the 'data segment':
	 * <ul>
	 * 	<li>'native'/Qortal 'trade' address - used as a MESSAGE contact</li>
	 * 	<li>'foreign'/BitcoinCash public key hash - used by Alice's P2SH scripts to allow redeem</li>
	 * 	<li>HASH160 of secret-B - used by AT and P2SH to validate a potential secret-B</li>
	 * 	<li>QORT amount on offer by Bob</li>
	 * 	<li>BCH amount expected in return by Bob (from Alice)</li>
	...
	public byte[] createTrade(Repository repository, TradeBotCreateRequest tradeBotCreateRequest) throws DataException {
		...
		// Convert BitcoinCash receiving address into public key hash (we only support P2PKH at this time)
		Address bitcoinCashReceivingAddress;
		try {
			bitcoinCashReceivingAddress = Address.fromString(BitcoinCash.getInstance().getNetworkParameters(), tradeBotCreateRequest.receivingAddress);
		} catch (AddressFormatException e) {
			throw new DataException("Unsupported BitcoinCash receiving address: " + tradeBotCreateRequest.receivingAddress);
		}
		if (bitcoinCashReceivingAddress.getOutputScriptType() != ScriptType.P2PKH)
			throw new DataException("Unsupported BitcoinCash receiving address: " + tradeBotCreateRequest.receivingAddress);

		byte[] bitcoinCashReceivingAccountInfo = bitcoinCashReceivingAddress.getHash();
		...
		String name = "QORT/BCH ACCT";
		String description = "QORT/BCH cross-chain trade";
		String aTType = "ACCT";
		String tags = "ACCT QORT BCH";
		byte[] creationBytes = BitcoinCashACCTv1.buildQortalAT(tradeNativeAddress, tradeForeignPublicKeyHash, hashOfSecretB, tradeBotCreateRequest.qortAmount,
		...
		TradeBotData tradeBotData =  new TradeBotData(tradePrivateKey, BitcoinCashACCTv1.NAME,
				State.BOB_WAITING_FOR_AT_CONFIRM.name(), State.BOB_WAITING_FOR_AT_CONFIRM.value,
				creator.getAddress(), atAddress, timestamp, tradeBotCreateRequest.qortAmount,
				tradeNativePublicKey, tradeNativePublicKeyHash, tradeNativeAddress,
				secretB, hashOfSecretB,
				SupportedBlockchain.BITCOINCASH.name(),
				tradeForeignPublicKey, tradeForeignPublicKeyHash,
				tradeBotCreateRequest.foreignAmount, null, null, null, bitcoinCashReceivingAccountInfo);
	...
	/**
	 * Creates a trade-bot entry from the 'Alice' viewpoint, i.e. matching BCH to an existing offer.
	 * <p>
	 * Requires a chosen trade offer from Bob, passed by <tt>crossChainTradeData</tt>
	 * and access to a BitcoinCash wallet via <tt>xprv58</tt>.
	 * <p>
	 * The <tt>crossChainTradeData</tt> contains the current trade offer state
	 * as extracted from the AT's data segment.
	 * <p>
	 * Access to a funded wallet is via a BitcoinCash BIP32 hierarchical deterministic key,
	 ...
	 * which should result in a base58 string starting with either 'xprv' (for BitcoinCash main-net)
	 * or 'tprv' for (BitcoinCash test-net).
	 * <p>
	 * It is envisaged that the value in <tt>xprv58</tt> will actually come from a Qortal-UI-managed wallet.
	 * <p>
	 * If sufficient funds are available, <b>this method will actually fund the P2SH-A</b>
	 * with the BitcoinCash amount expected by 'Bob'.
	 * <p>
	 * If the BitcoinCash transaction is successfully broadcast to the network then the trade-bot entry
	 ...
	 * @return true if P2SH-A funding transaction successfully broadcast to BitcoinCash network, false otherwise
	 * @throws DataException
	 */
	public ResponseResult startResponse(Repository repository, ATData atData, ACCT acct, CrossChainTradeData crossChainTradeData, String xprv58, String receivingAddress) throws DataException {
		...
		// Check we have enough funds via xprv58 to fund both P2SHs to cover expectedBitcoinCash
		String tradeForeignAddress = BitcoinCash.getInstance().pkhToAddress(tradeForeignPublicKeyHash);

		long p2shFee;
		try {
			p2shFee = BitcoinCash.getInstance().getP2shFee(now);
		} catch (ForeignBlockchainException e) {
			LOGGER.debug("Couldn't estimate BitcoinCash fees?");
		...
		Transaction fundingCheckTransaction = BitcoinCash.getInstance().buildSpend(xprv58, tradeForeignAddress, totalFundsRequired);
		if (fundingCheckTransaction == null)
			return ResponseResult.BALANCE_ISSUE;

		// P2SH-A to be funded
		byte[] redeemScriptBytes = BitcoinyHTLC.buildScript(tradeForeignPublicKeyHash, lockTimeA, crossChainTradeData.creatorForeignPKH, hashOfSecretA);
		String p2shAddress = BitcoinCash.getInstance().deriveP2shAddress(redeemScriptBytes);
		...
		Transaction p2shFundingTransaction = BitcoinCash.getInstance().buildSpend(tradeBotData.getForeignKey(), p2shAddress, amountA);
		...
		try {
			BitcoinCash.getInstance().broadcastTransaction(p2shFundingTransaction);
		} catch (ForeignBlockchainException e) {
	...
	public void progress(Repository repository, TradeBotData tradeBotData) throws DataException, ForeignBlockchainException {
		...
		if (tradeBotState.requiresAtData) {
			...
			if (tradeBotState.requiresTradeData) {
				tradeData = BitcoinCashACCTv1.getInstance().populateTradeData(repository, atData);
				if (tradeData == null) {
	 ...
	 * <ul>
	 * 	<li>Alice's 'foreign'/BitcoinCash public key hash - so Bob's trade-bot can derive P2SH-A address and check balance</li>
	...
	private void handleAliceWaitingForP2shA(Repository repository, TradeBotData tradeBotData,
		...
		String p2shAddressA = bitcoinCash.deriveP2shAddress(redeemScriptA);

		// Fee for redeem/refund is subtracted from P2SH-A balance.
		long feeTimestampA = calcP2shAFeeTimestamp(tradeBotData.getLockTimeA(), crossChainTradeData.tradeTimeout);
		long p2shFeeA = bitcoinCash.getP2shFee(feeTimestampA);
		long minimumAmountA = crossChainTradeData.expectedForeignAmount - P2SH_B_OUTPUT_AMOUNT + p2shFeeA;
		BitcoinyHTLC.Status htlcStatusA = BitcoinyHTLC.determineHtlcStatus(bitcoinCash.getBlockchainProvider(), p2shAddressA, minimumAmountA);
		...
		byte[] messageData = BitcoinCashACCTv1.buildOfferMessage(tradeBotData.getTradeForeignPublicKeyHash(), tradeBotData.getHashOfSecret(), tradeBotData.getLockTimeA());
	 ...
	 * <p>
	 * Assuming P2SH-A has at least expected BitcoinCash balance,
	...
	private void handleBobWaitingForMessage(Repository repository, TradeBotData tradeBotData,
		...
		while (!messageTransactionsData.isEmpty()) {
			...
			// We're expecting: HASH160(secret-A), Alice's BitcoinCash pubkeyhash and lockTime-A
			byte[] messageData = messageTransactionData.getData();
			BitcoinCashACCTv1.OfferMessageData offerMessageData = BitcoinCashACCTv1.extractOfferMessageData(messageData);
			if (offerMessageData == null)
				continue;

			byte[] aliceForeignPublicKeyHash = offerMessageData.partnerBitcoinCashPKH;
			...
			int lockTimeB = BitcoinCashACCTv1.calcLockTimeB(messageTransactionData.getTimestamp(), lockTimeA);

			// Build outgoing message, padding each part to 32 bytes to make it easier for AT to consume
			byte[] outgoingMessageData = BitcoinCashACCTv1.buildTradeMessage(aliceNativeAddress, aliceForeignPublicKeyHash, hashOfSecretA, lockTimeA, lockTimeB);
			...
			String p2shAddressB = bitcoinCash.deriveP2shAddress(redeemScriptB);
	 ...
	 * <p>
	 * If all is well, trade-bot then uses BitcoinCash wallet to (token) fund P2SH-B.
	 * <p>
	 * If P2SH-B funding transaction is successfully broadcast to the BitcoinCash network, trade-bot's next
	...
	private void handleAliceWaitingForAtLock(Repository repository, TradeBotData tradeBotData,
		...
		int lockTimeB = BitcoinCashACCTv1.calcLockTimeB(recipientMessageTimestamp, lockTimeA);
		...
		long p2shFeeB = bitcoinCash.getP2shFee(feeTimestampB);

		// Have we funded P2SH-B already?
		final long minimumAmountB = P2SH_B_OUTPUT_AMOUNT + p2shFeeB;

		BitcoinyHTLC.Status htlcStatusB = BitcoinyHTLC.determineHtlcStatus(bitcoinCash.getBlockchainProvider(), p2shAddressB, minimumAmountB);
		...
		switch (htlcStatusB) {
			case UNFUNDED: {
				...
				Transaction p2shFundingTransaction = bitcoinCash.buildSpend(tradeBotData.getForeignKey(), p2shAddressB, amountB);
				if (p2shFundingTransaction == null) {
					LOGGER.debug("Unable to build P2SH-B funding transaction - lack of funds?");
					return;
				}

				bitcoinCash.broadcastTransaction(p2shFundingTransaction);
				break;
			}
	...
	private void handleBobWaitingForP2shB(Repository repository, TradeBotData tradeBotData,
		...
		List<TransactionOutput> fundingOutputs = bitcoinCash.getUnspentOutputs(p2shAddressB);
		byte[] receivingAccountInfo = tradeBotData.getReceivingAccountInfo();

		Transaction p2shRedeemTransaction = BitcoinyHTLC.buildRedeemTransaction(bitcoinCash.getNetworkParameters(), redeemAmount, redeemKey,
				fundingOutputs, redeemScriptB, tradeBotData.getSecret(), receivingAccountInfo);

		bitcoinCash.broadcastTransaction(p2shRedeemTransaction);
	 ...
	 * trade address.
	 * <p>
	 * In revealing a valid secret-A, Bob can then redeem the BCH funds from P2SH-A.
	 * <p>
	...
	private void handleAliceWatchingP2shB(Repository repository, TradeBotData tradeBotData,
		...
		byte[] secretB = BitcoinyHTLC.findHtlcSecret(bitcoinCash, p2shAddressB);
		if (secretB == null)
		...
		byte[] messageData = BitcoinCashACCTv1.buildRedeemMessage(secretA, secretB, qortalReceivingAddress);
		String messageRecipient = tradeBotData.getAtAddress();
	...
	/**
	 * Trade-bot is waiting for Alice to redeem Bob's AT, thus revealing secret-A which is required to spend the BCH funds from P2SH-A.
	 * <p>
	 * It's possible that Bob's AT has reached its trading timeout and automatically refunded QORT back to Bob. In which case,
	 * trade-bot is done with this specific trade and finalizes in refunded state.
	 * <p>
	 * Assuming trade-bot can extract a valid secret-A from Alice's MESSAGE then trade-bot uses that to redeem the BCH funds from P2SH-A
	 * to Bob's 'foreign'/BitcoinCash trade legacy-format address, as derived from trade private key.
	 * <p>
	 * (This could potentially be 'improved' to send BCH to any address of Bob's choosing by changing the transaction output).
	 * <p>
	...
	private void handleBobWaitingForAtRedeem(Repository repository, TradeBotData tradeBotData,
		...
		byte[] secretA = BitcoinCashACCTv1.findSecretA(repository, crossChainTradeData);
		if (secretA == null) {
		...
		switch (htlcStatusA) {
			...
			case FUNDED: {
				...
				ECKey redeemKey = ECKey.fromPrivate(tradeBotData.getTradePrivateKey());
				List<TransactionOutput> fundingOutputs = bitcoinCash.getUnspentOutputs(p2shAddressA);
		...
		String receivingAddress = bitcoinCash.pkhToAddress(receivingAccountInfo);
	...
	private void handleAliceRefundingP2shB(Repository repository, TradeBotData tradeBotData,
		...
		int medianBlockTime = bitcoinCash.getMedianBlockTime();
		...
		switch (htlcStatusB) {
			...
			case FUNDED:{
				...
				String receiveAddress = bitcoinCash.getUnusedReceiveAddress(tradeBotData.getForeignKey());
				Address receiving = Address.fromString(bitcoinCash.getNetworkParameters(), receiveAddress);

				Transaction p2shRefundTransaction = BitcoinyHTLC.buildRefundTransaction(bitcoinCash.getNetworkParameters(), refundAmount, refundKey,
						fundingOutputs, redeemScriptB, lockTimeB, receiving.getHash());

				bitcoinCash.broadcastTransaction(p2shRefundTransaction);
				break;
			}
		}
