 ...
 * </ul>
 */
public class BitcoinGoldACCTv1TradeBot implements AcctTradeBot {

	private static final Logger LOGGER = LogManager.getLogger(BitcoinGoldACCTv1TradeBot.class);
	...
	private static BitcoinGoldACCTv1TradeBot instance;
	...
	private BitcoinGoldACCTv1TradeBot() {
	}

	public static synchronized BitcoinGoldACCTv1TradeBot getInstance() {
		if (instance == null)
			instance = new BitcoinGoldACCTv1TradeBot();
	...
	/**
	 * Creates a new trade-bot entry from the "Bob" viewpoint, i.e. OFFERing QORT in exchange for BTG.
	 * <p>
	 * Generates:
	 * <ul>
	 ...
	 * 	<li>'foreign' (as in BitcoinGold) public key, public key hash</li>
	 *	<li>HASH160 of secret-B</li>
	 * </ul>
	 * A Qortal AT is then constructed including the following as constants in the 'data segment':
	 * <ul>
	 * 	<li>'native'/Qortal 'trade' address - used as a MESSAGE contact</li>
	 * 	<li>'foreign'/BitcoinGold public key hash - used by Alice's P2SH scripts to allow redeem</li>
	 * 	<li>HASH160 of secret-B - used by AT and P2SH to validate a potential secret-B</li>
	 * 	<li>QORT amount on offer by Bob</li>
	 * 	<li>BTG amount expected in return by Bob (from Alice)</li>
	...
	public byte[] createTrade(Repository repository, TradeBotCreateRequest tradeBotCreateRequest) throws DataException {
		...
		// Convert BitcoinGold receiving address into public key hash (we only support P2PKH at this time)
		Address bitcoinGoldReceivingAddress;
		try {
			bitcoinGoldReceivingAddress = Address.fromString(BitcoinGold.getInstance().getNetworkParameters(), tradeBotCreateRequest.receivingAddress);
		} catch (AddressFormatException e) {
			throw new DataException("Unsupported BitcoinGold receiving address: " + tradeBotCreateRequest.receivingAddress);
		}
		if (bitcoinGoldReceivingAddress.getOutputScriptType() != ScriptType.P2PKH)
			throw new DataException("Unsupported BitcoinGold receiving address: " + tradeBotCreateRequest.receivingAddress);

		byte[] bitcoinGoldReceivingAccountInfo = bitcoinGoldReceivingAddress.getHash();
		...
		String name = "QORT/BTG ACCT";
		String description = "QORT/BTG cross-chain trade";
		String aTType = "ACCT";
		String tags = "ACCT QORT BTG";
		byte[] creationBytes = BitcoinGoldACCTv1.buildQortalAT(tradeNativeAddress, tradeForeignPublicKeyHash, hashOfSecretB, tradeBotCreateRequest.qortAmount,
		...
		TradeBotData tradeBotData =  new TradeBotData(tradePrivateKey, BitcoinGoldACCTv1.NAME,
				State.BOB_WAITING_FOR_AT_CONFIRM.name(), State.BOB_WAITING_FOR_AT_CONFIRM.value,
				creator.getAddress(), atAddress, timestamp, tradeBotCreateRequest.qortAmount,
				tradeNativePublicKey, tradeNativePublicKeyHash, tradeNativeAddress,
				secretB, hashOfSecretB,
				SupportedBlockchain.BITCOINGOLD.name(),
				tradeForeignPublicKey, tradeForeignPublicKeyHash,
				tradeBotCreateRequest.foreignAmount, null, null, null, bitcoinGoldReceivingAccountInfo);
	...
	/**
	 * Creates a trade-bot entry from the 'Alice' viewpoint, i.e. matching BTG to an existing offer.
	 * <p>
	 * Requires a chosen trade offer from Bob, passed by <tt>crossChainTradeData</tt>
	 * and access to a BitcoinGold wallet via <tt>xprv58</tt>.
	 * <p>
	 * The <tt>crossChainTradeData</tt> contains the current trade offer state
	 * as extracted from the AT's data segment.
	 * <p>
	 * Access to a funded wallet is via a BitcoinGold BIP32 hierarchical deterministic key,
	 ...
	 * which should result in a base58 string starting with either 'xprv' (for BitcoinGold main-net)
	 * or 'tprv' for (BitcoinGold test-net).
	 * <p>
	 * It is envisaged that the value in <tt>xprv58</tt> will actually come from a Qortal-UI-managed wallet.
	 * <p>
	 * If sufficient funds are available, <b>this method will actually fund the P2SH-A</b>
	 * with the BitcoinGold amount expected by 'Bob'.
	 * <p>
	 * If the BitcoinGold transaction is successfully broadcast to the network then the trade-bot entry
	 ...
	 * @return true if P2SH-A funding transaction successfully broadcast to BitcoinGold network, false otherwise
	 * @throws DataException
	 */
	public ResponseResult startResponse(Repository repository, ATData atData, ACCT acct, CrossChainTradeData crossChainTradeData, String xprv58, String receivingAddress) throws DataException {
		...
		// Check we have enough funds via xprv58 to fund both P2SHs to cover expectedBitcoinGold
		String tradeForeignAddress = BitcoinGold.getInstance().pkhToAddress(tradeForeignPublicKeyHash);

		long p2shFee;
		try {
			p2shFee = BitcoinGold.getInstance().getP2shFee(now);
		} catch (ForeignBlockchainException e) {
			LOGGER.debug("Couldn't estimate BitcoinGold fees?");
		...
		Transaction fundingCheckTransaction = BitcoinGold.getInstance().buildSpend(xprv58, tradeForeignAddress, totalFundsRequired);
		if (fundingCheckTransaction == null)
			return ResponseResult.BALANCE_ISSUE;

		// P2SH-A to be funded
		byte[] redeemScriptBytes = BitcoinyHTLC.buildScript(tradeForeignPublicKeyHash, lockTimeA, crossChainTradeData.creatorForeignPKH, hashOfSecretA);
		String p2shAddress = BitcoinGold.getInstance().deriveP2shAddress(redeemScriptBytes);
		...
		Transaction p2shFundingTransaction = BitcoinGold.getInstance().buildSpend(tradeBotData.getForeignKey(), p2shAddress, amountA);
		...
		try {
			BitcoinGold.getInstance().broadcastTransaction(p2shFundingTransaction);
		} catch (ForeignBlockchainException e) {
	...
	public void progress(Repository repository, TradeBotData tradeBotData) throws DataException, ForeignBlockchainException {
		...
		if (tradeBotState.requiresAtData) {
			...
			if (tradeBotState.requiresTradeData) {
				tradeData = BitcoinGoldACCTv1.getInstance().populateTradeData(repository, atData);
				if (tradeData == null) {
	 ...
	 * <ul>
	 * 	<li>Alice's 'foreign'/BitcoinGold public key hash - so Bob's trade-bot can derive P2SH-A address and check balance</li>
	...
	private void handleAliceWaitingForP2shA(Repository repository, TradeBotData tradeBotData,
		...
		String p2shAddressA = bitcoinGold.deriveP2shAddress(redeemScriptA);

		// Fee for redeem/refund is subtracted from P2SH-A balance.
		long feeTimestampA = calcP2shAFeeTimestamp(tradeBotData.getLockTimeA(), crossChainTradeData.tradeTimeout);
		long p2shFeeA = bitcoinGold.getP2shFee(feeTimestampA);
		long minimumAmountA = crossChainTradeData.expectedForeignAmount - P2SH_B_OUTPUT_AMOUNT + p2shFeeA;
		BitcoinyHTLC.Status htlcStatusA = BitcoinyHTLC.determineHtlcStatus(bitcoinGold.getBlockchainProvider(), p2shAddressA, minimumAmountA);
		...
		byte[] messageData = BitcoinGoldACCTv1.buildOfferMessage(tradeBotData.getTradeForeignPublicKeyHash(), tradeBotData.getHashOfSecret(), tradeBotData.getLockTimeA());
	 ...
	 * <p>
	 * Assuming P2SH-A has at least expected BitcoinGold balance,
	...
	private void handleBobWaitingForMessage(Repository repository, TradeBotData tradeBotData,
		...
		while (!messageTransactionsData.isEmpty()) {
			...
			// We're expecting: HASH160(secret-A), Alice's BitcoinGold pubkeyhash and lockTime-A
			byte[] messageData = messageTransactionData.getData();
			BitcoinGoldACCTv1.OfferMessageData offerMessageData = BitcoinGoldACCTv1.extractOfferMessageData(messageData);
			if (offerMessageData == null)
				continue;

			byte[] aliceForeignPublicKeyHash = offerMessageData.partnerBitcoinGoldPKH;
			...
			int lockTimeB = BitcoinGoldACCTv1.calcLockTimeB(messageTransactionData.getTimestamp(), lockTimeA);

			// Build outgoing message, padding each part to 32 bytes to make it easier for AT to consume
			byte[] outgoingMessageData = BitcoinGoldACCTv1.buildTradeMessage(aliceNativeAddress, aliceForeignPublicKeyHash, hashOfSecretA, lockTimeA, lockTimeB);
			...
			String p2shAddressB = bitcoinGold.deriveP2shAddress(redeemScriptB);
	 ...
	 * <p>
	 * If all is well, trade-bot then uses BitcoinGold wallet to (token) fund P2SH-B.
	 * <p>
	 * If P2SH-B funding transaction is successfully broadcast to the BitcoinGold network, trade-bot's next
	...
	private void handleAliceWaitingForAtLock(Repository repository, TradeBotData tradeBotData,
		...
		int lockTimeB = BitcoinGoldACCTv1.calcLockTimeB(recipientMessageTimestamp, lockTimeA);
		...
		long p2shFeeB = bitcoinGold.getP2shFee(feeTimestampB);

		// Have we funded P2SH-B already?
		final long minimumAmountB = P2SH_B_OUTPUT_AMOUNT + p2shFeeB;

		BitcoinyHTLC.Status htlcStatusB = BitcoinyHTLC.determineHtlcStatus(bitcoinGold.getBlockchainProvider(), p2shAddressB, minimumAmountB);
		...
		switch (htlcStatusB) {
			case UNFUNDED: {
				...
				Transaction p2shFundingTransaction = bitcoinGold.buildSpend(tradeBotData.getForeignKey(), p2shAddressB, amountB);
				if (p2shFundingTransaction == null) {
					LOGGER.debug("Unable to build P2SH-B funding transaction - lack of funds?");
					return;
				}

				bitcoinGold.broadcastTransaction(p2shFundingTransaction);
				break;
			}
	...
	private void handleBobWaitingForP2shB(Repository repository, TradeBotData tradeBotData,
		...
		List<TransactionOutput> fundingOutputs = bitcoinGold.getUnspentOutputs(p2shAddressB);
		byte[] receivingAccountInfo = tradeBotData.getReceivingAccountInfo();

		Transaction p2shRedeemTransaction = BitcoinyHTLC.buildRedeemTransaction(bitcoinGold.getNetworkParameters(), redeemAmount, redeemKey,
				fundingOutputs, redeemScriptB, tradeBotData.getSecret(), receivingAccountInfo);

		bitcoinGold.broadcastTransaction(p2shRedeemTransaction);
	 ...
	 * trade address.
	 * <p>
	 * In revealing a valid secret-A, Bob can then redeem the BTG funds from P2SH-A.
	 * <p>
	...
	private void handleAliceWatchingP2shB(Repository repository, TradeBotData tradeBotData,
		...
		byte[] secretB = BitcoinyHTLC.findHtlcSecret(bitcoinGold, p2shAddressB);
		if (secretB == null)
		...
		byte[] messageData = BitcoinGoldACCTv1.buildRedeemMessage(secretA, secretB, qortalReceivingAddress);
		String messageRecipient = tradeBotData.getAtAddress();
	...
	/**
	 * Trade-bot is waiting for Alice to redeem Bob's AT, thus revealing secret-A which is required to spend the BTG funds from P2SH-A.
	 * <p>
	 * It's possible that Bob's AT has reached its trading timeout and automatically refunded QORT back to Bob. In which case,
	 * trade-bot is done with this specific trade and finalizes in refunded state.
	 * <p>
	 * Assuming trade-bot can extract a valid secret-A from Alice's MESSAGE then trade-bot uses that to redeem the BTG funds from P2SH-A
	 * to Bob's 'foreign'/BitcoinGold trade legacy-format address, as derived from trade private key.
	 * <p>
	 * (This could potentially be 'improved' to send BTG to any address of Bob's choosing by changing the transaction output).
	 * <p>
	...
	private void handleBobWaitingForAtRedeem(Repository repository, TradeBotData tradeBotData,
		...
		byte[] secretA = BitcoinGoldACCTv1.findSecretA(repository, crossChainTradeData);
		if (secretA == null) {
		...
		switch (htlcStatusA) {
			...
			case FUNDED: {
				...
				ECKey redeemKey = ECKey.fromPrivate(tradeBotData.getTradePrivateKey());
				List<TransactionOutput> fundingOutputs = bitcoinGold.getUnspentOutputs(p2shAddressA);
		...
		String receivingAddress = bitcoinGold.pkhToAddress(receivingAccountInfo);
	...
	private void handleAliceRefundingP2shB(Repository repository, TradeBotData tradeBotData,
		...
		int medianBlockTime = bitcoinGold.getMedianBlockTime();
		...
		switch (htlcStatusB) {
			...
			case FUNDED:{
				...
				String receiveAddress = bitcoinGold.getUnusedReceiveAddress(tradeBotData.getForeignKey());
				Address receiving = Address.fromString(bitcoinGold.getNetworkParameters(), receiveAddress);

				Transaction p2shRefundTransaction = BitcoinyHTLC.buildRefundTransaction(bitcoinGold.getNetworkParameters(), refundAmount, refundKey,
						fundingOutputs, redeemScriptB, lockTimeB, receiving.getHash());

				bitcoinGold.broadcastTransaction(p2shRefundTransaction);
				break;
			}
		}
