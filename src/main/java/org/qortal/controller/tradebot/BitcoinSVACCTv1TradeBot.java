 ...
 * </ul>
 */
public class BitcoinSVACCTv1TradeBot implements AcctTradeBot {

	private static final Logger LOGGER = LogManager.getLogger(BitcoinSVACCTv1TradeBot.class);
	...
	private static BitcoinSVACCTv1TradeBot instance;
	...
	private BitcoinSVACCTv1TradeBot() {
	}

	public static synchronized BitcoinSVACCTv1TradeBot getInstance() {
		if (instance == null)
			instance = new BitcoinSVACCTv1TradeBot();
	...
	/**
	 * Creates a new trade-bot entry from the "Bob" viewpoint, i.e. OFFERing QORT in exchange for BSV.
	 * <p>
	 * Generates:
	 * <ul>
	 ...
	 * 	<li>'foreign' (as in BitcoinSV) public key, public key hash</li>
	 *	<li>HASH160 of secret-B</li>
	 * </ul>
	 * A Qortal AT is then constructed including the following as constants in the 'data segment':
	 * <ul>
	 * 	<li>'native'/Qortal 'trade' address - used as a MESSAGE contact</li>
	 * 	<li>'foreign'/BitcoinSV public key hash - used by Alice's P2SH scripts to allow redeem</li>
	 * 	<li>HASH160 of secret-B - used by AT and P2SH to validate a potential secret-B</li>
	 * 	<li>QORT amount on offer by Bob</li>
	 * 	<li>BSV amount expected in return by Bob (from Alice)</li>
	...
	public byte[] createTrade(Repository repository, TradeBotCreateRequest tradeBotCreateRequest) throws DataException {
		...
		// Convert BitcoinSV receiving address into public key hash (we only support P2PKH at this time)
		Address bitcoinSVReceivingAddress;
		try {
			bitcoinSVReceivingAddress = Address.fromString(BitcoinSV.getInstance().getNetworkParameters(), tradeBotCreateRequest.receivingAddress);
		} catch (AddressFormatException e) {
			throw new DataException("Unsupported BitcoinSV receiving address: " + tradeBotCreateRequest.receivingAddress);
		}
		if (bitcoinSVReceivingAddress.getOutputScriptType() != ScriptType.P2PKH)
			throw new DataException("Unsupported BitcoinSV receiving address: " + tradeBotCreateRequest.receivingAddress);

		byte[] bitcoinSVReceivingAccountInfo = bitcoinSVReceivingAddress.getHash();
		...
		String name = "QORT/BSV ACCT";
		String description = "QORT/BSV cross-chain trade";
		String aTType = "ACCT";
		String tags = "ACCT QORT BSV";
		byte[] creationBytes = BitcoinSVACCTv1.buildQortalAT(tradeNativeAddress, tradeForeignPublicKeyHash, hashOfSecretB, tradeBotCreateRequest.qortAmount,
		...
		TradeBotData tradeBotData =  new TradeBotData(tradePrivateKey, BitcoinSVACCTv1.NAME,
				State.BOB_WAITING_FOR_AT_CONFIRM.name(), State.BOB_WAITING_FOR_AT_CONFIRM.value,
				creator.getAddress(), atAddress, timestamp, tradeBotCreateRequest.qortAmount,
				tradeNativePublicKey, tradeNativePublicKeyHash, tradeNativeAddress,
				secretB, hashOfSecretB,
				SupportedBlockchain.BITCOINSV.name(),
				tradeForeignPublicKey, tradeForeignPublicKeyHash,
				tradeBotCreateRequest.foreignAmount, null, null, null, bitcoinSVReceivingAccountInfo);
	...
	/**
	 * Creates a trade-bot entry from the 'Alice' viewpoint, i.e. matching BSV to an existing offer.
	 * <p>
	 * Requires a chosen trade offer from Bob, passed by <tt>crossChainTradeData</tt>
	 * and access to a BitcoinSV wallet via <tt>xprv58</tt>.
	 * <p>
	 * The <tt>crossChainTradeData</tt> contains the current trade offer state
	 * as extracted from the AT's data segment.
	 * <p>
	 * Access to a funded wallet is via a BitcoinSV BIP32 hierarchical deterministic key,
	 ...
	 * which should result in a base58 string starting with either 'xprv' (for BitcoinSV main-net)
	 * or 'tprv' for (BitcoinSV test-net).
	 * <p>
	 * It is envisaged that the value in <tt>xprv58</tt> will actually come from a Qortal-UI-managed wallet.
	 * <p>
	 * If sufficient funds are available, <b>this method will actually fund the P2SH-A</b>
	 * with the BitcoinSV amount expected by 'Bob'.
	 * <p>
	 * If the BitcoinSV transaction is successfully broadcast to the network then the trade-bot entry
	 ...
	 * @return true if P2SH-A funding transaction successfully broadcast to BitcoinSV network, false otherwise
	 * @throws DataException
	 */
	public ResponseResult startResponse(Repository repository, ATData atData, ACCT acct, CrossChainTradeData crossChainTradeData, String xprv58, String receivingAddress) throws DataException {
		...
		// Check we have enough funds via xprv58 to fund both P2SHs to cover expectedBitcoinSV
		String tradeForeignAddress = BitcoinSV.getInstance().pkhToAddress(tradeForeignPublicKeyHash);

		long p2shFee;
		try {
			p2shFee = BitcoinSV.getInstance().getP2shFee(now);
		} catch (ForeignBlockchainException e) {
			LOGGER.debug("Couldn't estimate BitcoinSV fees?");
		...
		Transaction fundingCheckTransaction = BitcoinSV.getInstance().buildSpend(xprv58, tradeForeignAddress, totalFundsRequired);
		if (fundingCheckTransaction == null)
			return ResponseResult.BALANCE_ISSUE;

		// P2SH-A to be funded
		byte[] redeemScriptBytes = BitcoinyHTLC.buildScript(tradeForeignPublicKeyHash, lockTimeA, crossChainTradeData.creatorForeignPKH, hashOfSecretA);
		String p2shAddress = BitcoinSV.getInstance().deriveP2shAddress(redeemScriptBytes);
		...
		Transaction p2shFundingTransaction = BitcoinSV.getInstance().buildSpend(tradeBotData.getForeignKey(), p2shAddress, amountA);
		...
		try {
			BitcoinSV.getInstance().broadcastTransaction(p2shFundingTransaction);
		} catch (ForeignBlockchainException e) {
	...
	public void progress(Repository repository, TradeBotData tradeBotData) throws DataException, ForeignBlockchainException {
		...
		if (tradeBotState.requiresAtData) {
			...
			if (tradeBotState.requiresTradeData) {
				tradeData = BitcoinSVACCTv1.getInstance().populateTradeData(repository, atData);
				if (tradeData == null) {
	 ...
	 * <ul>
	 * 	<li>Alice's 'foreign'/BitcoinSV public key hash - so Bob's trade-bot can derive P2SH-A address and check balance</li>
	...
	private void handleAliceWaitingForP2shA(Repository repository, TradeBotData tradeBotData,
		...
		String p2shAddressA = bitcoinSV.deriveP2shAddress(redeemScriptA);

		// Fee for redeem/refund is subtracted from P2SH-A balance.
		long feeTimestampA = calcP2shAFeeTimestamp(tradeBotData.getLockTimeA(), crossChainTradeData.tradeTimeout);
		long p2shFeeA = bitcoinSV.getP2shFee(feeTimestampA);
		long minimumAmountA = crossChainTradeData.expectedForeignAmount - P2SH_B_OUTPUT_AMOUNT + p2shFeeA;
		BitcoinyHTLC.Status htlcStatusA = BitcoinyHTLC.determineHtlcStatus(bitcoinSV.getBlockchainProvider(), p2shAddressA, minimumAmountA);
		...
		byte[] messageData = BitcoinSVACCTv1.buildOfferMessage(tradeBotData.getTradeForeignPublicKeyHash(), tradeBotData.getHashOfSecret(), tradeBotData.getLockTimeA());
	 ...
	 * <p>
	 * Assuming P2SH-A has at least expected BitcoinSV balance,
	...
	private void handleBobWaitingForMessage(Repository repository, TradeBotData tradeBotData,
		...
		while (!messageTransactionsData.isEmpty()) {
			...
			// We're expecting: HASH160(secret-A), Alice's BitcoinSV pubkeyhash and lockTime-A
			byte[] messageData = messageTransactionData.getData();
			BitcoinSVACCTv1.OfferMessageData offerMessageData = BitcoinSVACCTv1.extractOfferMessageData(messageData);
			if (offerMessageData == null)
				continue;

			byte[] aliceForeignPublicKeyHash = offerMessageData.partnerBitcoinSVPKH;
			...
			int lockTimeB = BitcoinSVACCTv1.calcLockTimeB(messageTransactionData.getTimestamp(), lockTimeA);

			// Build outgoing message, padding each part to 32 bytes to make it easier for AT to consume
			byte[] outgoingMessageData = BitcoinSVACCTv1.buildTradeMessage(aliceNativeAddress, aliceForeignPublicKeyHash, hashOfSecretA, lockTimeA, lockTimeB);
			...
			String p2shAddressB = bitcoinSV.deriveP2shAddress(redeemScriptB);
	 ...
	 * <p>
	 * If all is well, trade-bot then uses BitcoinSV wallet to (token) fund P2SH-B.
	 * <p>
	 * If P2SH-B funding transaction is successfully broadcast to the BitcoinSV network, trade-bot's next
	...
	private void handleAliceWaitingForAtLock(Repository repository, TradeBotData tradeBotData,
		...
		int lockTimeB = BitcoinSVACCTv1.calcLockTimeB(recipientMessageTimestamp, lockTimeA);
		...
		long p2shFeeB = bitcoinSV.getP2shFee(feeTimestampB);

		// Have we funded P2SH-B already?
		final long minimumAmountB = P2SH_B_OUTPUT_AMOUNT + p2shFeeB;

		BitcoinyHTLC.Status htlcStatusB = BitcoinyHTLC.determineHtlcStatus(bitcoinSV.getBlockchainProvider(), p2shAddressB, minimumAmountB);
		...
		switch (htlcStatusB) {
			case UNFUNDED: {
				...
				Transaction p2shFundingTransaction = bitcoinSV.buildSpend(tradeBotData.getForeignKey(), p2shAddressB, amountB);
				if (p2shFundingTransaction == null) {
					LOGGER.debug("Unable to build P2SH-B funding transaction - lack of funds?");
					return;
				}

				bitcoinSV.broadcastTransaction(p2shFundingTransaction);
				break;
			}
	...
	private void handleBobWaitingForP2shB(Repository repository, TradeBotData tradeBotData,
		...
		List<TransactionOutput> fundingOutputs = bitcoinSV.getUnspentOutputs(p2shAddressB);
		byte[] receivingAccountInfo = tradeBotData.getReceivingAccountInfo();

		Transaction p2shRedeemTransaction = BitcoinyHTLC.buildRedeemTransaction(bitcoinSV.getNetworkParameters(), redeemAmount, redeemKey,
				fundingOutputs, redeemScriptB, tradeBotData.getSecret(), receivingAccountInfo);

		bitcoinSV.broadcastTransaction(p2shRedeemTransaction);
	 ...
	 * trade address.
	 * <p>
	 * In revealing a valid secret-A, Bob can then redeem the BSV funds from P2SH-A.
	 * <p>
	...
	private void handleAliceWatchingP2shB(Repository repository, TradeBotData tradeBotData,
		...
		byte[] secretB = BitcoinyHTLC.findHtlcSecret(bitcoinSV, p2shAddressB);
		if (secretB == null)
		...
		byte[] messageData = BitcoinSVACCTv1.buildRedeemMessage(secretA, secretB, qortalReceivingAddress);
		String messageRecipient = tradeBotData.getAtAddress();
	...
	/**
	 * Trade-bot is waiting for Alice to redeem Bob's AT, thus revealing secret-A which is required to spend the BSV funds from P2SH-A.
	 * <p>
	 * It's possible that Bob's AT has reached its trading timeout and automatically refunded QORT back to Bob. In which case,
	 * trade-bot is done with this specific trade and finalizes in refunded state.
	 * <p>
	 * Assuming trade-bot can extract a valid secret-A from Alice's MESSAGE then trade-bot uses that to redeem the BSV funds from P2SH-A
	 * to Bob's 'foreign'/BitcoinSV trade legacy-format address, as derived from trade private key.
	 * <p>
	 * (This could potentially be 'improved' to send BSV to any address of Bob's choosing by changing the transaction output).
	 * <p>
	...
	private void handleBobWaitingForAtRedeem(Repository repository, TradeBotData tradeBotData,
		...
		byte[] secretA = BitcoinSVACCTv1.findSecretA(repository, crossChainTradeData);
		if (secretA == null) {
		...
		switch (htlcStatusA) {
			...
			case FUNDED: {
				...
				ECKey redeemKey = ECKey.fromPrivate(tradeBotData.getTradePrivateKey());
				List<TransactionOutput> fundingOutputs = bitcoinSV.getUnspentOutputs(p2shAddressA);
		...
		String receivingAddress = bitcoinSV.pkhToAddress(receivingAccountInfo);
	...
	private void handleAliceRefundingP2shB(Repository repository, TradeBotData tradeBotData,
		...
		int medianBlockTime = bitcoinSV.getMedianBlockTime();
		...
		switch (htlcStatusB) {
			...
			case FUNDED:{
				...
				String receiveAddress = bitcoinSV.getUnusedReceiveAddress(tradeBotData.getForeignKey());
				Address receiving = Address.fromString(bitcoinSV.getNetworkParameters(), receiveAddress);

				Transaction p2shRefundTransaction = BitcoinyHTLC.buildRefundTransaction(bitcoinSV.getNetworkParameters(), refundAmount, refundKey,
						fundingOutputs, redeemScriptB, lockTimeB, receiving.getHash());

				bitcoinSV.broadcastTransaction(p2shRefundTransaction);
				break;
			}
		}
