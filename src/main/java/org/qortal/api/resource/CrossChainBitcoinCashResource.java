...
public class CrossChainBitcoinCashResource {
	...
	public String getBitcoinCashWalletBalance(String key58) {
		...
		Long balance = bitcoinCash.getWalletBalance(key58);
		if (balance == null)
	...
	@ApiErrors({ApiError.INVALID_PRIVATE_KEY, ApiError.FOREIGN_BLOCKCHAIN_NETWORK_ISSUE})
	public List<SimpleTransaction> getBitcoinCashWalletTransactions(String key58) {
		Security.checkApiCallAllowed(request);
	...
	public String sendBitcoin(BitcoinCashSendRequest bitcoinCashSendRequest) {
		Security.checkApiCallAllowed(request);
