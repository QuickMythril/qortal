...
public class CrossChainBitcoinResource {
	...
	public RepairWalletPreview previewRepairOldWallet(@HeaderParam(Security.API_KEY_HEADER) String apiKey, String key58) {
		...
		try {
			return bitcoin.previewRepairOldWallet(key58);
		} catch (ForeignBlockchainException e) {
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.FOREIGN_BLOCKCHAIN_NETWORK_ISSUE);
		}
	}
	...
	public String repairOldWallet(@HeaderParam(Security.API_KEY_HEADER) String apiKey, @QueryParam("force") Boolean force, String key58) {
		...
		if (!bitcoin.isValidDeterministicPrivateKey(key58))
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_PRIVATE_KEY);

		try {
			if (force == null || !force) {
				RepairWalletPreview preview = bitcoin.previewRepairOldWallet(key58);
			...
			return bitcoin.repairOldWallet(key58);
