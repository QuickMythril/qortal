...
@Path("/crosschain/BitcoinCashACCTv1")
@Tag(name = "Cross-Chain (BitcoinCashACCTv1)")
public class CrossChainBitcoinCashACCTv1Resource {
	...
	@Path("/build")
	@Operation(
		summary = "Build BitcoinCash cross-chain trading AT",
	...
	public String buildTrade(CrossChainBuildRequest tradeRequest) {
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			byte[] creationBytes = BitcoinCashACCTv1.buildQortalAT(creatorAccount.getAddress(), tradeRequest.bitcoinPublicKeyHash, tradeRequest.hashOfSecretB,
			...
			long fee = 0;
			String name = "QORT-BCH cross-chain trade";
			String description = "Qortal-BitcoinCash cross-chain trade";
			String atType = "ACCT";
			String tags = "QORT-BCH ACCT";
	...
	public String buildTradeMessage(CrossChainTradeRequest tradeRequest) {
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			CrossChainTradeData crossChainTradeData = BitcoinCashACCTv1.getInstance().populateTradeData(repository, atData);
	...
	public String buildRedeemMessage(CrossChainDualSecretRequest secretRequest) {
		...
		if (secretRequest.secretA == null || secretRequest.secretA.length != BitcoinCashACCTv1.SECRET_LENGTH)
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_DATA);

		if (secretRequest.secretB == null || secretRequest.secretB.length != BitcoinCashACCTv1.SECRET_LENGTH)
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_DATA);
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			byte[] messageData = BitcoinCashACCTv1.buildRedeemMessage(secretRequest.secretA, secretRequest.secretB, secretRequest.receivingAddress);
	...
	private ATData fetchAtDataWithChecking(Repository repository, String atAddress) throws DataException {
		...
		if (!Arrays.equals(atData.getCodeHash(), BitcoinCashACCTv1.CODE_BYTES_HASH))
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_CRITERIA);
