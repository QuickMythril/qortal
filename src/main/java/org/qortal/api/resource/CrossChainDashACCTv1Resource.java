...
@Path("/crosschain/DashACCTv1")
@Tag(name = "Cross-Chain (DashACCTv1)")
public class CrossChainDashACCTv1Resource {
	...
	public boolean buildRedeemMessage(@HeaderParam(Security.API_KEY_HEADER) String apiKey, CrossChainSecretRequest secretRequest) {
		...
		if (secretRequest.secret == null || secretRequest.secret.length != DashACCTv1.SECRET_LENGTH)
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			CrossChainTradeData crossChainTradeData = DashACCTv1.getInstance().populateTradeData(repository, atData);
			...
			byte[] messageData = DashACCTv1.buildRedeemMessage(secretRequest.secret, secretRequest.receivingAddress);
	...
	private ATData fetchAtDataWithChecking(Repository repository, String atAddress) throws DataException {
		...
		if (!Arrays.equals(atData.getCodeHash(), DashACCTv1.CODE_BYTES_HASH))
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_CRITERIA);
