...
@Path("/crosschain/PeercoinACCTv1")
@Tag(name = "Cross-Chain (PeercoinACCTv1)")
public class CrossChainPeercoinACCTv1Resource {
	...
	public boolean buildRedeemMessage(@HeaderParam(Security.API_KEY_HEADER) String apiKey, CrossChainSecretRequest secretRequest) {
		...
		if (secretRequest.secret == null || secretRequest.secret.length != PeercoinACCTv1.SECRET_LENGTH)
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			CrossChainTradeData crossChainTradeData = PeercoinACCTv1.getInstance().populateTradeData(repository, atData);
			...
			byte[] messageData = PeercoinACCTv1.buildRedeemMessage(secretRequest.secret, secretRequest.receivingAddress);
	...
	private ATData fetchAtDataWithChecking(Repository repository, String atAddress) throws DataException {
		...
		if (!Arrays.equals(atData.getCodeHash(), PeercoinACCTv1.CODE_BYTES_HASH))
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_CRITERIA);
