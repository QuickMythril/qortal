...
@Path("/crosschain/BitcoinSVACCTv1")
@Tag(name = "Cross-Chain (BitcoinSVACCTv1)")
public class CrossChainBitcoinSVACCTv1Resource {
	...
	@Path("/build")
	@Operation(
		summary = "Build BitcoinSV cross-chain trading AT",
	...
	public String buildTrade(CrossChainBuildRequest tradeRequest) {
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			byte[] creationBytes = BitcoinSVACCTv1.buildQortalAT(creatorAccount.getAddress(), tradeRequest.bitcoinPublicKeyHash, tradeRequest.hashOfSecretB,
			...
			long fee = 0;
			String name = "QORT-BSV cross-chain trade";
			String description = "Qortal-BitcoinSV cross-chain trade";
			String atType = "ACCT";
			String tags = "QORT-BSV ACCT";
	...
	public String buildTradeMessage(CrossChainTradeRequest tradeRequest) {
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			CrossChainTradeData crossChainTradeData = BitcoinSVACCTv1.getInstance().populateTradeData(repository, atData);
	...
	public String buildRedeemMessage(CrossChainDualSecretRequest secretRequest) {
		...
		if (secretRequest.secretA == null || secretRequest.secretA.length != BitcoinSVACCTv1.SECRET_LENGTH)
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_DATA);

		if (secretRequest.secretB == null || secretRequest.secretB.length != BitcoinSVACCTv1.SECRET_LENGTH)
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_DATA);
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			byte[] messageData = BitcoinSVACCTv1.buildRedeemMessage(secretRequest.secretA, secretRequest.secretB, secretRequest.receivingAddress);
	...
	private ATData fetchAtDataWithChecking(Repository repository, String atAddress) throws DataException {
		...
		if (!Arrays.equals(atData.getCodeHash(), BitcoinSVACCTv1.CODE_BYTES_HASH))
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_CRITERIA);
