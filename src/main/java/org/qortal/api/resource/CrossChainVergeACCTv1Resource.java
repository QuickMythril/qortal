...
@Path("/crosschain/VergeACCTv1")
@Tag(name = "Cross-Chain (VergeACCTv1)")
public class CrossChainVergeACCTv1Resource {
	...
	public boolean buildRedeemMessage(@HeaderParam(Security.API_KEY_HEADER) String apiKey, CrossChainSecretRequest secretRequest) {
		...
		if (secretRequest.secret == null || secretRequest.secret.length != VergeACCTv1.SECRET_LENGTH)
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			CrossChainTradeData crossChainTradeData = VergeACCTv1.getInstance().populateTradeData(repository, atData);
			...
			byte[] messageData = VergeACCTv1.buildRedeemMessage(secretRequest.secret, secretRequest.receivingAddress);
	...
	private ATData fetchAtDataWithChecking(Repository repository, String atAddress) throws DataException {
		...
		if (!Arrays.equals(atData.getCodeHash(), VergeACCTv1.CODE_BYTES_HASH))
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_CRITERIA);
