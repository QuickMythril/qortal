...
@Path("/crosschain/KomodoACCTv1")
@Tag(name = "Cross-Chain (KomodoACCTv1)")
public class CrossChainKomodoACCTv1Resource {
	...
	public boolean buildRedeemMessage(@HeaderParam(Security.API_KEY_HEADER) String apiKey, CrossChainSecretRequest secretRequest) {
		...
		if (secretRequest.secret == null || secretRequest.secret.length != KomodoACCTv1.SECRET_LENGTH)
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			CrossChainTradeData crossChainTradeData = KomodoACCTv1.getInstance().populateTradeData(repository, atData);
			...
			byte[] messageData = KomodoACCTv1.buildRedeemMessage(secretRequest.secret, secretRequest.receivingAddress);
	...
	private ATData fetchAtDataWithChecking(Repository repository, String atAddress) throws DataException {
		...
		if (!Arrays.equals(atData.getCodeHash(), KomodoACCTv1.CODE_BYTES_HASH))
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_CRITERIA);
