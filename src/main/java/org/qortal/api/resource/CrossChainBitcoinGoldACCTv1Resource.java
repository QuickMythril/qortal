...
@Path("/crosschain/BitcoinGoldACCTv1")
@Tag(name = "Cross-Chain (BitcoinGoldACCTv1)")
public class CrossChainBitcoinGoldACCTv1Resource {
	...
	@Path("/build")
	@Operation(
		summary = "Build BitcoinGold cross-chain trading AT",
	...
	public String buildTrade(CrossChainBuildRequest tradeRequest) {
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			byte[] creationBytes = BitcoinGoldACCTv1.buildQortalAT(creatorAccount.getAddress(), tradeRequest.bitcoinPublicKeyHash, tradeRequest.hashOfSecretB,
			...
			long fee = 0;
			String name = "QORT-BTG cross-chain trade";
			String description = "Qortal-BitcoinGold cross-chain trade";
			String atType = "ACCT";
			String tags = "QORT-BTG ACCT";
	...
	public String buildTradeMessage(CrossChainTradeRequest tradeRequest) {
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			CrossChainTradeData crossChainTradeData = BitcoinGoldACCTv1.getInstance().populateTradeData(repository, atData);
	...
	public String buildRedeemMessage(CrossChainDualSecretRequest secretRequest) {
		...
		if (secretRequest.secretA == null || secretRequest.secretA.length != BitcoinGoldACCTv1.SECRET_LENGTH)
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_DATA);

		if (secretRequest.secretB == null || secretRequest.secretB.length != BitcoinGoldACCTv1.SECRET_LENGTH)
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_DATA);
		...
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			byte[] messageData = BitcoinGoldACCTv1.buildRedeemMessage(secretRequest.secretA, secretRequest.secretB, secretRequest.receivingAddress);
	...
	private ATData fetchAtDataWithChecking(Repository repository, String atAddress) throws DataException {
		...
		if (!Arrays.equals(atData.getCodeHash(), BitcoinGoldACCTv1.CODE_BYTES_HASH))
			throw ApiExceptionFactory.INSTANCE.createException(request, ApiError.INVALID_CRITERIA);
