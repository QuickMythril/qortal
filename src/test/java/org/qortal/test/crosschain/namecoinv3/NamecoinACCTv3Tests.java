...
public class NamecoinACCTv3Tests extends Common {
	...
	public static final long namecoinAmount = 864200L; // 0.00864200 NMC
	...
	public void testCompile() {
		...
		byte[] creationBytes = NamecoinACCTv3.buildQortalAT(tradeAccount.getAddress(), namecoinPublicKeyHash, redeemAmount, namecoinAmount, tradeTimeout);
	...
	public void testOfferCancel() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			byte[] messageData = NamecoinACCTv3.getInstance().buildCancelMessage(deployer.getAddress());
			...
			CrossChainTradeData tradeData = NamecoinACCTv3.getInstance().populateTradeData(repository, atData);
	...
	public void testTradingInfoProcessing() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			int refundTimeout = NamecoinACCTv3.calcRefundTimeout(partnersOfferMessageTransactionTimestamp, lockTimeA);

			// Send trade info to AT
			byte[] messageData = NamecoinACCTv3.buildTradeMessage(partner.getAddress(), namecoinPublicKeyHash, hashOfSecretA, lockTimeA, refundTimeout);
			...
			// Check trade partner's namecoin PKH was extracted correctly
			assertTrue(Arrays.equals(namecoinPublicKeyHash, tradeData.partnerForeignPKH));
	...
	public void testCorrectSecretCorrectSender() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			messageData = NamecoinACCTv3.buildRedeemMessage(secretA,  partner.getAddress());
	...
	public void testIncorrectSecretCorrectSender() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			messageData = NamecoinACCTv3.buildRedeemMessage(wrongSecret, partner.getAddress());
			messageTransaction = sendMessage(repository, partner, messageData, atAddress);
	...
	public void testDescribeDeployed() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			for (ATData atData : executableAts) {
				...
				// Not one of ours?
				if (!Arrays.equals(codeHash, NamecoinACCTv3.CODE_BYTES_HASH))
					continue;
	...
	private DeployAtTransaction doDeploy(Repository repository, PrivateKeyAccount deployer, String tradeAddress) throws DataException {
		byte[] creationBytes = NamecoinACCTv3.buildQortalAT(tradeAddress, namecoinPublicKeyHash, redeemAmount, namecoinAmount, tradeTimeout);
		...
		Long fee = null;
		String name = "QORT-NMC cross-chain trade";
		String description = String.format("Qortal-Namecoin cross-chain trade");
		String atType = "ACCT";
		String tags = "QORT-NMC ACCT";
