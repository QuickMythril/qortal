...
public class DashACCTv3Tests extends Common {
	...
	public static final long dashAmount = 864200L; // 0.00864200 DASH
	...
	public void testCompile() {
		...
		byte[] creationBytes = DashACCTv3.buildQortalAT(tradeAccount.getAddress(), dashPublicKeyHash, redeemAmount, dashAmount, tradeTimeout);
	...
	public void testOfferCancel() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			byte[] messageData = DashACCTv3.getInstance().buildCancelMessage(deployer.getAddress());
			...
			CrossChainTradeData tradeData = DashACCTv3.getInstance().populateTradeData(repository, atData);
	...
	public void testTradingInfoProcessing() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			int refundTimeout = DashACCTv3.calcRefundTimeout(partnersOfferMessageTransactionTimestamp, lockTimeA);

			// Send trade info to AT
			byte[] messageData = DashACCTv3.buildTradeMessage(partner.getAddress(), dashPublicKeyHash, hashOfSecretA, lockTimeA, refundTimeout);
			...
			// Check trade partner's dash PKH was extracted correctly
			assertTrue(Arrays.equals(dashPublicKeyHash, tradeData.partnerForeignPKH));
	...
	public void testCorrectSecretCorrectSender() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			messageData = DashACCTv3.buildRedeemMessage(secretA,  partner.getAddress());
	...
	public void testIncorrectSecretCorrectSender() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			messageData = DashACCTv3.buildRedeemMessage(wrongSecret, partner.getAddress());
			messageTransaction = sendMessage(repository, partner, messageData, atAddress);
	...
	public void testDescribeDeployed() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			for (ATData atData : executableAts) {
				...
				// Not one of ours?
				if (!Arrays.equals(codeHash, DashACCTv3.CODE_BYTES_HASH))
					continue;
	...
	private DeployAtTransaction doDeploy(Repository repository, PrivateKeyAccount deployer, String tradeAddress) throws DataException {
		byte[] creationBytes = DashACCTv3.buildQortalAT(tradeAddress, dashPublicKeyHash, redeemAmount, dashAmount, tradeTimeout);
		...
		Long fee = null;
		String name = "QORT-DASH cross-chain trade";
		String description = String.format("Qortal-Dash cross-chain trade");
		String atType = "ACCT";
		String tags = "QORT-DASH ACCT";
