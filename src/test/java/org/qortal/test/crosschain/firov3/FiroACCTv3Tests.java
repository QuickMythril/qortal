...
public class FiroACCTv3Tests extends Common {
	...
	public static final long firoAmount = 864200L; // 0.00864200 FIRO
	...
	public void testCompile() {
		...
		byte[] creationBytes = FiroACCTv3.buildQortalAT(tradeAccount.getAddress(), firoPublicKeyHash, redeemAmount, firoAmount, tradeTimeout);
	...
	public void testOfferCancel() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			byte[] messageData = FiroACCTv3.getInstance().buildCancelMessage(deployer.getAddress());
			...
			CrossChainTradeData tradeData = FiroACCTv3.getInstance().populateTradeData(repository, atData);
	...
	public void testTradingInfoProcessing() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			int refundTimeout = FiroACCTv3.calcRefundTimeout(partnersOfferMessageTransactionTimestamp, lockTimeA);

			// Send trade info to AT
			byte[] messageData = FiroACCTv3.buildTradeMessage(partner.getAddress(), firoPublicKeyHash, hashOfSecretA, lockTimeA, refundTimeout);
			...
			// Check trade partner's firo PKH was extracted correctly
			assertTrue(Arrays.equals(firoPublicKeyHash, tradeData.partnerForeignPKH));
	...
	public void testCorrectSecretCorrectSender() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			messageData = FiroACCTv3.buildRedeemMessage(secretA,  partner.getAddress());
	...
	public void testIncorrectSecretCorrectSender() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			messageData = FiroACCTv3.buildRedeemMessage(wrongSecret, partner.getAddress());
			messageTransaction = sendMessage(repository, partner, messageData, atAddress);
	...
	public void testDescribeDeployed() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			for (ATData atData : executableAts) {
				...
				// Not one of ours?
				if (!Arrays.equals(codeHash, FiroACCTv3.CODE_BYTES_HASH))
					continue;
	...
	private DeployAtTransaction doDeploy(Repository repository, PrivateKeyAccount deployer, String tradeAddress) throws DataException {
		byte[] creationBytes = FiroACCTv3.buildQortalAT(tradeAddress, firoPublicKeyHash, redeemAmount, firoAmount, tradeTimeout);
		...
		Long fee = null;
		String name = "QORT-FIRO cross-chain trade";
		String description = String.format("Qortal-Firo cross-chain trade");
		String atType = "ACCT";
		String tags = "QORT-FIRO ACCT";
