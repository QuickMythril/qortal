package org.qortal.test.crosschain.peercoinv3;
...
import org.qortal.crosschain.PeercoinACCTv3;
...
public class PeercoinACCTv3Tests extends Common {
	...
	public static final byte[] peercoinPublicKeyHash = HashCode.fromString("bb00bb11bb22bb33bb44bb55bb66bb77bb88bb99").asBytes();
	public static final int tradeTimeout = 20; // blocks
	public static final long redeemAmount = 80_40200000L;
	public static final long fundingAmount = 123_45600000L;
	public static final long peercoinAmount = 864200L; // 0.00864200 PPC
	...
	public void testCompile() {
		...
		byte[] creationBytes = PeercoinACCTv3.buildQortalAT(tradeAccount.getAddress(), peercoinPublicKeyHash, redeemAmount, peercoinAmount, tradeTimeout);
	...
	public void testOfferCancel() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			byte[] messageData = PeercoinACCTv3.getInstance().buildCancelMessage(deployer.getAddress());
			...
			CrossChainTradeData tradeData = PeercoinACCTv3.getInstance().populateTradeData(repository, atData);
	...
	public void testTradingInfoProcessing() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			int refundTimeout = PeercoinACCTv3.calcRefundTimeout(partnersOfferMessageTransactionTimestamp, lockTimeA);

			// Send trade info to AT
			byte[] messageData = PeercoinACCTv3.buildTradeMessage(partner.getAddress(), peercoinPublicKeyHash, hashOfSecretA, lockTimeA, refundTimeout);
			...
			// Check trade partner's peercoin PKH was extracted correctly
			assertTrue(Arrays.equals(peercoinPublicKeyHash, tradeData.partnerForeignPKH));
	...
	public void testCorrectSecretCorrectSender() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			messageData = PeercoinACCTv3.buildRedeemMessage(secretA,  partner.getAddress());
	...
	public void testIncorrectSecretCorrectSender() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			messageData = PeercoinACCTv3.buildRedeemMessage(wrongSecret, partner.getAddress());
			messageTransaction = sendMessage(repository, partner, messageData, atAddress);
	...
	public void testDescribeDeployed() throws DataException {
		try (final Repository repository = RepositoryManager.getRepository()) {
			...
			for (ATData atData : executableAts) {
				...
				// Not one of ours?
				if (!Arrays.equals(codeHash, PeercoinACCTv3.CODE_BYTES_HASH))
					continue;
	...
	private DeployAtTransaction doDeploy(Repository repository, PrivateKeyAccount deployer, String tradeAddress) throws DataException {
		byte[] creationBytes = PeercoinACCTv3.buildQortalAT(tradeAddress, peercoinPublicKeyHash, redeemAmount, peercoinAmount, tradeTimeout);
		...
		Long fee = null;
		String name = "QORT-PPC cross-chain trade";
		String description = String.format("Qortal-Peercoin cross-chain trade");
		String atType = "ACCT";
		String tags = "QORT-PPC ACCT";
